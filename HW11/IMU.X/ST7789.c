...
// drawChar function
void drawChar(unsigned short x, unsigned short y, unsigned short color, int letter) {
    // Checking bounds
    if ((x >= 0) & (y >= 0) & (x+5 < 240) & (y+8 < 240)) {
        int i, j; // i to loop through bytes, j for bits in each byte
        int ascii_num = letter - 32; // Start at 0x20
        for (i=0; i<5; i+=1) {
            char byte = ASCII[ascii_num][i];
            for (j=0; j<8; j+=1) {
                if (byte & (1<<j)) {
