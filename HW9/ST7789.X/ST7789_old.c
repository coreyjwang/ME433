...
// drawChar function
void drawChar(unsigned short x0, unsigned short y0, unsigned short color, int character){
    int x, y;
    for(x=0;x < 5; x++){
...
// drawChar function
//void drawChar(unsigned short x_start, unsigned short y_start, unsigned short color, int letter) {
//    int i, j; // i to loop through bytes, j for bits in each byte
////    int ascii_num = letter - 32; // Start at 0x20
//    for (i=0; i<5; i++) {
//        char byte = ASCII[letter - 32][i];
//        for (j=0; j<8; j++) {
//            if(byte & (1<<y_start)) {
//                LCD_drawPixel(x_start+i, y_start+j, color);
//            }
//        }
//    }
//}
...
void drawString(unsigned short x, unsigned short y, unsigned short color, char* m) {
    int length_of_m = sizeof m / sizeof m[0];
    int i; // Iterator for length of message
    for (i=0; i<length_of_m; i++) {
        drawChar(x, y, color, m[i]);
    }
}